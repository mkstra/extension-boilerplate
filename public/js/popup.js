...
var app = (function () {
    ...
    /* src/components/Table.svelte generated by Svelte v3.5.4 */
    ...
    function create_if_block(ctx) {
    	...
    	return {
    		c: function create() {
    			...
    			add_location(tr, file, 28, 3, 476);
    			add_location(thead, file, 27, 2, 465);
    			add_location(tbody, file, 35, 2, 622);
    			attr(table, "class", "svelte-115sffn");
    			add_location(table, file, 26, 1, 455);
    		},
    ...
    function create_if_block_1(ctx) {
    	...
    	return {
    		c: function create() {
    			...
    			add_location(button, file, 42, 8, 759);
    ...
    function create_if_block_2(ctx) {
    	...
    	return {
    		c: function create() {
    			...
    			button1.textContent = "📝 Find essays in History (last 60 days)";
    			t8 = space();
 ...
 Amazon pages searched. If nothing happens, it's because of too many fetch requests to Amazon (which denies then)` : '-')) {
    ...
    function instance$1($$self, $$props, $$invalidate) {
    	...
    	chrome.storage.onChanged.addListener((_c, _ns) => {
    		$$invalidate('collection', collection = getStorage());
    	});
    	...
    	const onRemove = async ({ detail }) => await chromePromise$1.storage.sync.remove(detail.url);
    	...
    	const onAdd = async ({ detail }) => {
    		...
    		await chromePromise$1.storage.sync.set({ [node.url]: node });
    	...
    	$$self.$$.update = ($$dirty = { collection: 1 }) => {
    		if ($$dirty.collection) { $$invalidate('link', link = JSONDownloadable(collection)); }
    	};
